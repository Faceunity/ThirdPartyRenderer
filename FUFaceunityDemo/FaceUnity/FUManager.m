...
@property(nonatomic, strong) NSMutableDictionary *beautifyValueDict;
...
-(void)setupFilterData{
    ...
    _filters = [NSMutableArray arrayWithCapacity:76];
    ...
    for (NSString *str in beautyFiltersDataSource) {
        ...
        modle.mValue = [[self.beautifyValueDict valueForKey:str] floatValue];
        [_filters addObject:modle];
    }

    // 取出最后一次选中的滤镜
    NSString *fliterStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"seletedFliter"];
    if (fliterStr.length) { // 有值
    
        for (int i = 0; i < _filters.count; i++) {
            
            FUBeautyParam *model = _filters[i];
            if ([fliterStr isEqualToString:model.mParam]) {
                
                self.seletedFliter = model;
                break;
            }
        }
...
-(void)setupSkinData{
    ...
    _skinParams = [NSMutableArray arrayWithCapacity:8];
...
-(void)setupShapData{
    ...
    _shapeParams = [NSMutableArray arrayWithCapacity:15];
